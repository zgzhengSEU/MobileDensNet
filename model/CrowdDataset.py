...
class CrowdDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        gt_dmap = np.load(os.path.join(self.gt_dmap_root,
        ...
        if self.gt_downsample > 1:
            ...
            # convert to order (channel,rows,cols)
...
class CrowdDataset_p2(Dataset):
    ...
    def __getitem__(self, index):
        ...
        gt_dmap = np.load(os.path.join(self.gt_dmap_root,
                          img_name.replace('.jpg', '.npy')))
        gt_dmap_p2 = np.load(os.path.join(self.gt_dmap_root,
                          img_name.replace('.jpg', '.npy')))
        if random.randint(0, 1) == 1 and self.phase == 'train':
            img = img[:, ::-1]  # 水平翻转
            gt_dmap = gt_dmap[:, ::-1]  # 水平翻转
        ...
        if self.gt_downsample > 1:
            ds_rows = int(img.shape[0]//self.gt_downsample)
            ds_cols = int(img.shape[1]//self.gt_downsample)
            ds_rows_p2 = int(img.shape[0] // 4)
            ds_cols_p2 = int(img.shape[1] // 4)
            img = cv2.resize(img, (ds_cols*self.gt_downsample,
                             ds_rows*self.gt_downsample))
            # convert to order (channel,rows,cols)
            img = img.transpose((2, 0, 1))
            ...
            gt_dmap = gt_dmap[np.newaxis, :, :] * \
                self.gt_downsample*self.gt_downsample
            gt_dmap_p2 = cv2.resize(gt_dmap_p2, (ds_cols_p2, ds_rows_p2))
            gt_dmap_p2 = gt_dmap_p2[np.newaxis, :, :] * \
                4*4
            img_tensor = torch.tensor(img, dtype=torch.float)
            img_tensor = transforms.functional.normalize(img_tensor, mean=[0.485, 0.456, 0.406],
                                                         std=[0.229, 0.224, 0.225])
            gt_dmap_tensor = torch.tensor(gt_dmap, dtype=torch.float)
            gt_dmap_tensor_p2 = torch.tensor(gt_dmap_p2, dtype=torch.float)
        return img_tensor, gt_dmap_tensor, gt_dmap_tensor_p2
