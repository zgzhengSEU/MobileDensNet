...
class BasicRFB(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride=1, scale = 0.1):
        ...
        self.scale = scale
        self.out_channels = out_planes
        self.inter_planes = in_planes // 8
        self.branch1 = nn.Sequential(
                BasicConv(in_planes, self.inter_planes, kernel_size=1, stride=1),
                BasicConv(self.inter_planes, (self.inter_planes//2)*3, kernel_size=(1,3), stride=1, padding=(0,1)),
                BasicConv((self.inter_planes//2)*3, (self.inter_planes//2)*3, kernel_size=(3,1), stride=stride, padding=(1,0)),
                BasicSepConv((self.inter_planes//2)*3, kernel_size=3, stride=1, padding=3, dilation=3, relu=False)
                )
        self.branch2 = nn.Sequential(
                BasicConv(in_planes, self.inter_planes, kernel_size=1, stride=1),
                BasicConv(self.inter_planes, (self.inter_planes//2)*3, kernel_size=3, stride=1, padding=1),
                BasicConv((self.inter_planes//2)*3, (self.inter_planes//2)*3, kernel_size=3, stride=stride, padding=1),
                BasicSepConv((self.inter_planes//2)*3, kernel_size=3, stride=1, padding=5, dilation=5, relu=False)
                )

        self.ConvLinear = BasicConv(3*self.inter_planes, out_planes, kernel_size=1, stride=1, relu=False)
...
class BasicRFB_a(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride=1, scale = 0.1):
        ...
        self.branch0 = nn.Sequential(
                BasicConv(in_planes, self.inter_planes, kernel_size=1, stride=1),
                BasicSepConv(self.inter_planes, kernel_size=3, stride=1, padding=1, dilation=1, relu=False)
                )
        self.branch1 = nn.Sequential(
                BasicConv(in_planes, self.inter_planes, kernel_size=1, stride=1),
                BasicConv(self.inter_planes, self.inter_planes, kernel_size=(3,1), stride=1, padding=(1,0)),
                BasicSepConv(self.inter_planes, kernel_size=3, stride=1, padding=3, dilation=3, relu=False)
                )
        self.branch2 = nn.Sequential(
                BasicConv(in_planes, self.inter_planes, kernel_size=1, stride=1),
                BasicConv(self.inter_planes, self.inter_planes, kernel_size=(1,3), stride=stride, padding=(0,1)),
                BasicSepConv(self.inter_planes, kernel_size=3, stride=1, padding=3, dilation=3, relu=False)
                )
        self.branch3 = nn.Sequential(
                BasicConv(in_planes, self.inter_planes//2, kernel_size=1, stride=1),
                BasicConv(self.inter_planes//2, (self.inter_planes//4)*3, kernel_size=(1,3), stride=1, padding=(0,1)),
                BasicConv((self.inter_planes//4)*3, self.inter_planes, kernel_size=(3,1), stride=stride, padding=(1,0)),
                BasicSepConv(self.inter_planes, kernel_size=3, stride=1, padding=5, dilation=5, relu=False)
                )

        self.ConvLinear = BasicConv(4*self.inter_planes, out_planes, kernel_size=1, stride=1, relu=False)
        self.relu = nn.ReLU(inplace=False)
...
class GhostNetV2P3_RFB(nn.Module):
    def __init__(self, FEM_kernel_size=1, use_dilation=False, width=1.0, block=GhostBottleneckV2, args=None):
        super(GhostNetV2P3_RFB, self).__init__()
        self.cfgs = [
            # k, t, c, SE, s
            # ====== p1 ==============
            # stage 0
