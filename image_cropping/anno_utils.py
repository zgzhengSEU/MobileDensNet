def format_label(anno_path, mode="train"):
    """
    返回 list(num * [bbox_left, bbox_top, bbox_right, bbox_bottom, raw_each_bbox_lable])
    ...
    coord_list = []
    for idx, raw_each_bbox_lable in enumerate(txt_list):
        coord_raw = [int(x) for x in raw_each_bbox_lable.replace("\n", "").split(',') if len(x) != 0]
        coord = coord_raw[:6]
        if len(coord) != 6:
            # 4 coord + 1 class
        ...
        coord_list.append([bbox_left, bbox_top, bbox_right, bbox_bottom, raw_each_bbox_lable])
    return coord_list
