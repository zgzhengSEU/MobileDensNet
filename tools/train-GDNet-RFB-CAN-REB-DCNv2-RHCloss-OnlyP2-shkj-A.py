...
def main(args):
    ...
    wandb_name='train-GDNet-RFB-CAN-REB-DCNv2-RHCloss-OnlyP2'
    # ===================== configuration ======================
    ...
    # ======================== dataloader =================================================================
    train_dataset = CrowdDataset(train_image_root, train_dmap_root, gt_downsample=4, phase='train')
    test_dataset = CrowdDataset(test_image_root, test_dmap_root, gt_downsample=4, phase='test')
...
def train_one_epoch_single_gpu_p2loc_rhc(model,
    ...
    for step, (img, gt_dmap_p2) in enumerate(train_loader):
        optimizer.zero_grad()
        img = img.to(device)
        ...
        if use_amp:
            ...
            with autocast():
                ...
                loss = criterion(et_dmap_p2, gt_dmap_p2)
            ...
            # calculate loss
            et_dmap_p2 = F.interpolate(et_dmap_p2, size=(gt_dmap_p2.shape[2], gt_dmap_p2.shape[3]), mode='bilinear', align_corners=True)
            loss_p2 = criterion(et_dmap_p2, gt_dmap_p2)
            eph = 0
            # hcmode = 0 # 相对
            hcmode = 1 # 绝对
            if epoch >= eph:
                ...
                if hcmode == 0:
                    et_count_p2 = et_count_p2 / (gt_count_p2 + 1)
                    gt_count_p2 = gt_count_p2 / (gt_count_p2 + 1)
                    loss_hc_p2 = criterion(et_count_p2, gt_count_p2)
                elif hcmode == 1:
                    y = 1e-3
                    loss_hc_p2 = y * criterion(et_count_p2, gt_count_p2)
                loss = loss_p2 + loss_hc_p2
            else:
                loss = loss_p2
            loss.backward()
        # update mean losses
        ...
        # 打印loss
        train_loader.desc = "[epoch {}] loss {}, loss_p2 {}, {} {}".format(
            epoch, round(mean_loss.item(), 3), round(mean_loss_p2.item(), 3),
...
def evaluate_single_gpu_p2loc(model,
    ...
    for step, (img, gt_dmap_p2) in enumerate(test_loader):
        img = img.to(device)
        gt_dmap_p2 = gt_dmap_p2.to(device)
        # forward propagation
        et_dmap, et_dmap_p2 = model(img)
        diff = abs(et_dmap_p2.data.sum() - gt_dmap_p2.data.sum())
        mae += diff
        mse += diff * diff
        ...
        if step == index and show_images:
            ...
            plt.axis('off')
            plt.imshow(img_np)
            gt_dmap_np = gt_dmap_p2[0].permute(1, 2, 0).cpu().numpy()
            ...
            plt.axis('off')
            plt.imshow(img_np)
            et_dmap_np = et_dmap_p2[0].permute(1, 2, 0).cpu().numpy()
        ...
        del img, gt_dmap_p2, et_dmap, et_dmap_p2
